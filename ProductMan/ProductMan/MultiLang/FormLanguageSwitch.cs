...
namespace ProductMan.MultiLang
{
    public class FormLanguageSwitchSingleton
    {
                ...
                if (tool != null)
                {
                    foreach (ToolStripItem item in tool.Items)
                    {
                        if (item is ToolStripButton)
                        {
                            ...
                            if (btn != null)
                                btn.Text = (string)GetSafeValue(resources, btn.Name + ".Text", btn.Text);
                        }
                    }
                }
            }
        ...
        protected virtual void ScanNonControls(System.Windows.Forms.ContainerControl containerControl, System.Resources.ResourceManager resources)
        {
            ...
            for (int i = 0; i < fieldInfo.Length; i++)
            {
                ...
                if (obj is System.Windows.Forms.ToolStripStatusLabel)
                {
                    System.Windows.Forms.ToolStripStatusLabel toolStripLabel = (System.Windows.Forms.ToolStripStatusLabel)obj;
                    toolStripLabel.Text = resources.GetString(fieldName + ".Text", m_cultureInfo);
                }
            }
        }
